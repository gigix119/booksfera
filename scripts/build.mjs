...
// Files excluded from the build (local dev tools, etc.)
const EXCLUDE_FROM_BUILD = new Set(["booksfera-generator.html"]);

// Auto-discover all HTML pages in the project root
const pages = {};
for (const file of fs.readdirSync(root).filter(f => f.endsWith(".html") && !EXCLUDE_FROM_BUILD.has(f))) {
  pages[file] = { path: file === "index.html" ? "/" : `/${file}` };
}
...
if (siteUrl) {
  const locs = Object.values(pages).map(p => p.path);
  const body = locs
    .map((p) => {
